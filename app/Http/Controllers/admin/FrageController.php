...
class FrageController extends Controller
{
    ...
    {
        echo "这里改变了";
        echo "GIT DEMO   新题发布页面";die;
        $selData = json_decode(DB::table('frageType')->select('unGrade')->get(),true);   
        ...
        $selData2 = json_decode(DB::table('frageType')->select('unCurriculum')->get(),true);   
        foreach ($selData2 as $k2 => $v2) {
            $v2 = join(",",$v2); //降维
            $temp2[$k2] = $v2;
        } 
        $temp2 = array_values(array_unique($temp2));

        $selData3 = json_decode(DB::table('frageType')->select('unUnid')->get(),true);   
        foreach ($selData3 as $k3 => $v3) {
            $v3 = join(",",$v3); //降维
            $temp3[$k3] = $v3;
        } 
        $temp3 = array_values(array_unique($temp3)); 

        $data['unGrade'] = $temp;
        $data['unCurriculum'] = $temp2;
        $data['unUnid'] = $temp3; 
        print_r($data);die;
        // $data['type'] = ['unGrade','unCurriculum','unUnid'];

        return view('admin.frage.addFrage',['frageType'=>$data]);
    }

    /**
     * 新题类型发布 - 获取传过来的二维数组，将数据组合成树形数组并返回
     */
